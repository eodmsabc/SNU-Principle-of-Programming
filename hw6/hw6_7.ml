...
*)
	...
	let rec leftitem l1 l2 =
		match l1 with
		| [] -> []
		| h :: t ->
			if (List.mem h l2) then leftitem t l2
			else h :: (leftitem t l2)
	in
...
(*			let res = eval e stl in
		...
		| SEQUENCE (c1, c2) ->
			...
			let (nnr, nnl) = insertf stl nr nl in
			exv c2 nnr nnl
		| REPEAT c ->
		...
		| EQ (e, c) ->
			...
			in
			let nl = eqv e stl in
			let left = leftitem stl nl in
			let (rrr, lll) = exv c nl l in
			(rrr @ left, lll)
		| NEQ (e, c) ->
			let rec neqv e lst =
				match lst with
